...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/verify-login", async (req, res) => {
    try {
      ...
      // Track user login history first
  ...
  app.post("/api/admin/reschedule-session", isAdmin, async (req, res) => {
    try {
      ...
      console.log(`Admin rescheduling - checking for time slot conflicts (email: ${session.email}, sessionId: ${session.id})`);
      const isBooked = await isTimeSlotBooked(parsedDate, session.id, session.email);
      if (isBooked) {
        return res.status(400).json({
          success: false,
...
// Razorpay Payment Routes
  ...
  app.post("/api/razorpay/webhook", async (req, res) => {
    try {
      ...
      const isValid = verifyWebhookSignature(JSON.stringify(webhookBody), webhookSignature);
      ...
      switch(event) {
        ...
        case 'payment.captured':
          await handleSuccessfulPaymentWebhook(webhookBody.payload.payment.entity);
          break;
        case 'payment.failed':
          console.log('Payment failed webhook received', webhookBody.payload.payment.entity.id);
          break;
        default:
